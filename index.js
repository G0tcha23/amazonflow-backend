      ...
      case 'new_order_flow':
        if (state.step === 1) {
          ...
          if (!msg.photo || msg.photo.length === 0) {
            await bot.sendMessage(chatId, '❌ Debes enviar una foto.\n\nIntenta de nuevo:', {
              reply_markup: {
                inline_keyboard: [[{ text: '❌ Cancelar', callback_data: 'cancel' }]]
              }
            });
    ...
    switch(data) {
      ...
      case 'send_review':
        ...
        if (userOrders.length === 0) {
          await bot.sendMessage(chatId, '⚠️ No tienes pedidos pendientes.');
          await showMainMenu(chatId, username);
          return;
        }
      ...
      case 'my_status':
        ...
        if (userOrdersList.length === 0) {
          ...
          if (user && user.intermediaries && user.intermediaries.length > 0) {
